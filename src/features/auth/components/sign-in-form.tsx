  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex items-center justify-between text-sm">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" className="rounded border-border-default text-accent-deep focus:ring-accent-deep" />
            <span className="text-text-secondary">Remember me</span>
          </label>
          <Link href="/auth/forgot-password" className="font-medium text-accent-deep hover:text-accent-green transition-colors">
            Forgot password?
          </Link>
        </div>
      ...
      <p className="mt-6 text-center text-sm text-text-secondary">
        ...
        <Link href={`/auth/sign-up${returnTo ? `?returnTo=${encodeURIComponent(returnTo)}` : ''}`} className="font-medium text-accent-deep hover:text-accent-green transition-colors">
          Sign up
        </Link>
      </p>
    </>

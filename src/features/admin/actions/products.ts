...
export async function createProductAction(payload: AdminProductFormContext) {
  ...
  const { data: newProduct, error: productError } = await supabase
    ...
    .insert({
      ...
      packaging_type: data.packaging_type || 'BAG | BOX (Paid)',
      production_capacity_per_day: data.production_capacity_per_day || null,
      delivery_time: data.delivery_time || '1 to 7 Days',

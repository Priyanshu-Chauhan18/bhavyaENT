...
import { deleteCatalogAction } from '@/features/admin/actions/catalogs';
...
export default async function AdminCatalogsPage() {
  const catalogs = await getAdminCatalogsList();

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Catalogs Hub</h1>
          <p className="text-sm text-slate-500 mt-1">Manage top-level categorical aggregations.</p>
        </div>
        <Link 
          href={`${ROUTES.ADMIN_CATALOGS}/new`}
              ...
              {catalogs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    No catalogs instantiated yet. Click &quot;Add Catalog&quot; to begin building.
                  </td>
                </tr>
              ) : (
                catalogs.map((cat) => (
                  <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="inline-flex items-center gap-1">
                        <Link 
                          href={`${ROUTES.ADMIN_CATALOGS}/${cat.id}/edit`}
                        ...
                        <DeleteButton
                          ...
                          returnPath={ROUTES.ADMIN_CATALOGS}
                        />
                      </div>
                    </td>
                  </tr>
